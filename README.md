...
```bash
uv tool install ssl_py_platform
```

This installs a single `ssl-py-platform` command, available from any project
...
ssl-py-platform build-native     # for compiled-extension (pybind11/scikit-build-core) projects
ssl-py-platform publish          # uv-based publish, requires UV_PUBLISH_* env vars
ssl-py-platform publish-ci       # twine-based publish, requires TWINE_* env vars
...
```just
setup:
    ssl-py-platform setup-env --extras dev,lint,tests,type-checking,pre-commit
```
...
- `ssl_py_platform` (this repo) holds the **local-dev** side: the same logic,
...
call `ssl_py_platform` themselves, so there's a single source of truth used by
